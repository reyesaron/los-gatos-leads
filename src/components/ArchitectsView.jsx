...
const CITIES = ["Los Gatos", "Saratoga", "San Jose", "Woodside", "Atherton", "Los Altos", "Los Altos Hills", "Palo Alto", "Milpitas", "Morgan Hill", "Campbell"];
...
export default function ArchitectsView({ crmData, scored }) {
  ...
  useEffect(() => {
    fetch("/api/architects").then(r => r.json()).then(d => setArchitects(d.architects || [])).catch(() => {}).finally(() => setLoading(false));
  }, []);

  // Auto-detect architects from CRM contact data
  const crmArchitects = useMemo(() => {
    const map = {};
    for (const p of scored) {
      const crm = crmData[p._leadId];
      if (crm?.contactRole === "Architect" && crm.contactName) {
  ...
  // Merge manual + CRM architects, dedup by name
  const allArchitects = useMemo(() => {
    const merged = [...architects.map(a => ({ ...a, projectCount: 0, projects: [], source: "manual" }))];
    for (const ca of crmArchitects) {
    ...
    return merged;
  }, [architects, crmArchitects]);

  const filtered = useMemo(() => {
    if (!search) return allArchitects;
    const q = search.toLowerCase();
    return allArchitects.filter(a => a.name.toLowerCase().includes(q) || (a.firm || "").toLowerCase().includes(q) || (a.specialty || "").toLowerCase().includes(q) || (a.cities || []).some(c => c.toLowerCase().includes(q)));
  }, [allArchitects, search]);
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch("/api/architects", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  ...
  const deleteArch = async (id) => {
    try {
      const res = await fetch("/api/architects", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "delete", id }) });
      const data = await res.json();
      if (data.architects) setArchitects(data.architects);
    } catch {}
  };
  ...
  if (loading) return <div style={{ padding: 40, textAlign: "center", color: MUTED }}>Loading architects...</div>;
    ...
    <div style={{ maxWidth: 980, margin: "0 auto", padding: "14px 20px 40px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: TEXT, flex: 1 }}>
          Architect & Designer Database
          <span style={{ fontSize: 12, color: MUTED, fontWeight: 400, marginLeft: 8 }}>{allArchitects.length} contacts</span>
        </div>
      ...
      {showForm && (
        <div style={{ background: CARD, borderRadius: 8, border: `1px solid ${BORDER}`, padding: 16, marginBottom: 12 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 11, color: MUTED, marginBottom: 3, fontWeight: 600 }}>Firm</div>
              <input value={form.firm} onChange={e => setForm(f => ({ ...f, firm: e.target.value }))} placeholder="Smith Architecture" style={iS} />
            </div>
            <div>
            ...
            <div>
              ...
              <input value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="jane@smitharch.com" style={iS} />
            </div>
            <div>
              <div style={{ fontSize: 11, color: MUTED, marginBottom: 3, fontWeight: 600 }}>Specialty</div>
              <input value={form.specialty} onChange={e => setForm(f => ({ ...f, specialty: e.target.value }))} placeholder="Custom homes, ADUs, historic..." style={iS} />
            </div>
            <div>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 12 }}>
            <button onClick={submit} disabled={!form.name.trim() || saving} style={{ padding: "7px 18px", borderRadius: 5, border: "none", background: form.name.trim() ? RED : "#1c1c1c", color: "#fff", fontSize: 12, fontWeight: 600, cursor: form.name.trim() ? "pointer" : "default", opacity: form.name.trim() ? 1 : 0.4 }}>
              {saving ? "Saving..." : editId ? "Update" : "Add Architect"}
            </button>
          </div>
        </div>
      )}
      ...
      {/* Architect Cards */}
      {filtered.length === 0 && <div style={{ textAlign: "center", padding: 40, color: MUTED }}>No architects yet. Add one or set contact role to "Architect" on a lead.</div>}
      {filtered.map((a, i) => (
        <div key={a.id || i} style={{ background: CARD, borderRadius: 6, border: `1px solid ${BORDER}`, padding: "10px 14px", marginBottom: 4, display: "flex", gap: 12, alignItems: "flex-start" }}>
          ...
          {a.source !== "crm" && a.id && (
            <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
              ...
              <button onClick={() => deleteArch(a.id)} style={{ padding: "4px 8px", borderRadius: 4, border: `1px solid ${BORDER}`, background: BG, color: DIM, fontSize: 10, cursor: "pointer" }}>Del</button>
            </div>
          )}
        </div>
      ))}
